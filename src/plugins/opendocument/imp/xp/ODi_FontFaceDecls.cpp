...
void ODi_FontFaceDecls::startElement (const gchar* pName,
    ...
    if (!strcmp(pName, "style:font-face")) {
        ...
        if (pFontFamily && (pFontFamily[0] == '\'') && (pFontFamily[strlen(pFontFamily)-1] == '\'')) {
            ...
            m_fontFamilies.ins(pStyleName,
                               fontFamily.substr(1, fontFamily.length()-2));
        } else {
            m_fontFamilies.ins(pStyleName, fontFamily);
        }
    }
}
 ...
 */
const UT_UTF8String* ODi_FontFaceDecls::getFontFamily(
