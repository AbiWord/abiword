    ...
    if (!strcmp(pName, "text:section" )) {
        ...
        if (pStyle && (pStyle->isAutomatic())) {
            ...
            if (pStyle->getParent() != NULL) {
                m_headingStyles.ins(UT_UTF8String(pOutlineLevel),
                                    pStyle->getParent()->getDisplayName());
            } else {
...
void ODi_TextContent_ListenerState::_defineAbiTOCHeadingStyles() {
    ...
    UT_UTF8String str;
    UT_UTF8String props;
    const UT_UTF8String* pStyleName;
    bool ok;
    ...
    for (i=0; i<count; i++) {
        ...
        for (j=1; j<5; j++) {
            ...
            pStyleName = m_headingStyles[str];
