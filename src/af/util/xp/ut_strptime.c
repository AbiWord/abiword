...
static int UT_strncasecmp (const char *s1, const char *s2, size_t n)
{
	...
	while ((ret = ((unsigned char) tolower (*s1) - (unsigned char) tolower (*s2))) == 0 && *s1++)
	{
		if (--n == 0)
			return 0;
		++s2;
...
# define match_string(cs1, s2) \
	...
	int result = UT_strncasecmp ((cs1), (s2), len) == 0;			      \
	if (result) (s2) += len;						      \
	result; })
#else
/* Oh come on.  Get a reasonable compiler.  */
# define match_string(cs1, s2) \
	(UT_strncasecmp ((cs1), (s2), strlen (cs1)) ? 0 : ((s2) += strlen (cs1), 1))
#endif
...
{								      \
	...
	if (UT_strncasecmp (alts, rp, len) == 0)			      \
	break;							      \
	alts += len + 1;						      \
	++val;							      \
}								      \
...
#define recursive(new_fmt) \
	(*(new_fmt) != '\0'							      \
	&& (rp = UT_strptime_internal (rp, (new_fmt), tm, decided)) != NULL)
...
# define UT_strncasecmp(s1, s2, n) __UT_strncasecmp (s1, s2, n)
#else
...
const unsigned short int __mon_yday[13][13] =
{
	/* Normal years.  */
...
internal_function
#endif
UT_strptime_internal (const char *buf, const char *format, struct tm *tm, enum locale_status *decided);
