  ...
  if (where == NULL)
  ...
  else
    {
      if (FIRST_STRING_P (where))
        {
          for (this_char = where - string1; this_char < (unsigned)size1; this_char++)
            printchar (string1[this_char]);

          where = string2;    
        }

      for (this_char = where - string2; this_char < (unsigned)size2; this_char++)
  ...
  if (debug)
    {
      ...
      for (debug_count = (unsigned)0; debug_count < (unsigned)size; debug_count++)
...
#define PUSH_FAILURE_POINT(pattern_place, string_place, failure_code)	\
  do {									\
    ...
    PUSH_FAILURE_ITEM (highest_active_reg);				\
									\
    DEBUG_PRINT2 ("  Pushing pattern 0x%x: ", (unsigned int)pattern_place);		\
    DEBUG_PRINT_COMPILED_PATTERN (bufp, pattern_place, pend);		\
    PUSH_FAILURE_ITEM (pattern_place);					\
									\
    DEBUG_PRINT2 ("  Pushing string 0x%x: `", (unsigned int)string_place);		\
...
{									\
  ...
  if (string_temp != NULL)						\
    str = (const char *) string_temp;					\
									\
  DEBUG_PRINT2 ("  Popping string 0x%x: `", (unsigned int)str);			\
  DEBUG_PRINT_DOUBLE_STRING (str, string1, size1, string2, size2);	\
  DEBUG_PRINT1 ("'\n");							\
  pat = (unsigned char *) POP_FAILURE_ITEM ();				\
  DEBUG_PRINT2 ("  Popping pattern 0x%x: ", (unsigned int)pat);			\
