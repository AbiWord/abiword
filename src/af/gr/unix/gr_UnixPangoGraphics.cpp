...
class GR_UnixPangoItem: public GR_Item
{
  ...
  public:
	...
	virtual GR_ScriptType getType() const {return (GR_ScriptType) GRScriptType_Undefined;} //!!!WDG need to return a real value
	virtual GR_Item *     makeCopy() const {return new GR_UnixPangoItem(pango_item_copy(m_pi), m_text.utf8_str());} // make a copy of this item
...
{
  public:
	GR_UnixPangoRenderInfo(GR_ScriptType type):
		GR_RenderInfo(type),
	    //m_pIndices(NULL),
	    //m_pVisAttr(NULL),
		m_iIndicesSize(0),
...
#if 0
SCRIPT_LOGATTR *GR_UnixPangoRenderInfo::s_pLogAttr           = NULL;
WCHAR *         GR_UnixPangoRenderInfo::s_pChars             = NULL;
#endif
int *           GR_UnixPangoRenderInfo::s_pAdvances          = NULL;
int *           GR_UnixPangoRenderInfo::s_pJustifiedAdvances = NULL;
int *           GR_UnixPangoRenderInfo::s_pJustify           = NULL;
UT_uint32       GR_UnixPangoRenderInfo::s_iInstanceCount     = 0;
UT_uint32       GR_UnixPangoRenderInfo::s_iAdvancesSize      = 0;
GR_RenderInfo * GR_UnixPangoRenderInfo::s_pOwnerDraw         = NULL;
GR_RenderInfo * GR_UnixPangoRenderInfo::s_pOwnerCP           = NULL;
GR_RenderInfo * GR_UnixPangoRenderInfo::s_pOwnerChar         = NULL;
...
/*
GR_UnixPangoGraphics::GR_UnixPangoGraphics(HDC hdc, HWND hwnd, XAP_App * pApp)
...
GR_UnixPangoGraphics::GR_UnixPangoGraphics(HDC hdc, const DOCINFO * pDI, XAP_App * pApp,
										 HGLOBAL hDevMode)
	:GR_UnixGraphics(hdc, pDI, pApp, hDevMode)
{
	if(!_constructorCommonCode())
	{
		// we should only get here if exceptions were not enabled
...
GR_Graphics *   GR_UnixPangoGraphics::graphicsAllocator(GR_AllocInfo& info)
{
	...
	try
	{
		...
		{
			// printer graphics required
			return new GR_UnixPangoGraphics(AI.m_hdc, AI.m_pDocInfo,
										   AI.m_pApp,AI.m_hDevMode);
		}
		else
		{
			// screen graphics required
			return new GR_UnixPangoGraphics(AI.m_hdc, AI.m_hwnd, AI.m_pApp);
		}
	}
	catch (usp_exception &e)
	{
		UT_DEBUGMSG(("GR_UnixPangoGraphics::graphicsAllocator: error 0x%04x\n",e.error));
		return NULL;
	}
	#endif
	catch (std::exception &e)
	{
		UT_DEBUGMSG(("GR_UnixPangoGraphics::graphicsAllocator: %s\n",e.what()));
		return NULL;
	}
	catch (...)
	{
		UT_DEBUGMSG(("GR_UnixPangoGraphics::graphicsAllocator: unknown error\n"));
		return NULL;
	}
}

#define GRUNIXPANGO_CHARBUFF_SIZE 100
...
bool GR_UnixPangoGraphics::itemize(UT_TextIterator & text, GR_Itemization & I)
{
	...
	if(iLen > GRUNIXPANGO_CHARBUFF_SIZE)
	{
		UT_DEBUGMSG(("GR_UnixPangoGraphics::itemize: text buffer too small (iLen %d)\n", iLen));
		pInChars = new gunichar[iLen];
	...
	for(i = 0; i < iLen; ++i, ++text)
	{
		...
		//This is just a quick first pass - need to make it much more efficient
		pInChars[i] = text.getChar();
	}
	//converting to UTF8 for pango
	glong itemsRead, itemsWritten;
	
	gchar * pUTF8text = g_ucs4_to_utf8(pInChars, iLen, &itemsRead, &itemsWritten, NULL /*GError **error*/);
	///!!!|WDG
	// GError ** ???
	// does this var text hang around for long enough?
	...
	*/
	...
	I.addItem(iPosEnd - iPosStart + 1, new GR_UnixPangoItem(GRScriptType_Void));
...
bool GR_UnixPangoGraphics::shape(GR_ShapingInfo & si, GR_RenderInfo *& ri)
{
	...
	static wchar_t wcInChars[GRUNIXPANGO_CHARBUFF_SIZE]; 
	wchar_t *pInChars = &wcInChars[0];
	bool bDeleteChars = false;      // using static buffer

	if(si.m_iLength > GRUNIXPANGO_CHARBUFF_SIZE)
	{
		UT_DEBUGMSG(("GR_UnixPangoGraphics::shape: char buffer too small (len %d)\n", si.m_iLength));
		pInChars = new wchar_t[si.m_iLength];
		UT_return_val_if_fail(pInChars,false);
	...
	for(i = 0; i < si.m_iLength; ++i, ++si.m_Text)
	{
		...
		pInChars[i] = (wchar_t)si.m_Text.getChar();
	}
	...
	*/
	...
	pango_shape(pItem->m_text.utf8_str(), pItem->m_text.byteLength(), &(pItem->m_pi->analysis), RI->m_pGlyphs);
	...
	*/
	...
	// once we implement the GR_UnixPangoRenderInfo::append(), etc., we
...
bool GR_UnixPangoGraphics::_needsSpecialBreaking(GR_UnixPangoRenderInfo &ri)
{
...
bool GR_UnixPangoGraphics::_needsSpecialCaretPositioning(GR_UnixPangoRenderInfo &ri)
{
	UT_return_val_if_fail(s_ppScriptProperties && ri.m_pItem, false);
	if(ri.m_bShapingFailed)
		return false;
...
bool GR_UnixPangoRenderInfo::split (GR_RenderInfo *&pri, bool bReverse)
{
	...
	GR_UnixPangoRenderInfo & RI = (GR_UnixPangoRenderInfo &) *pri;
	...
	GR_UnixPangoItem &I = (GR_UnixPangoItem &)*m_pItem;
	if(I.isRTL())
	{
...
{
	//sGR_UnixPangoGraphics::fScriptFreeCache(&m_sc);
};
...
#define abi_plugin_register abipgn_grunixpango_register
#define abi_plugin_unregister abipgn_grunixpango_unregister
#define abi_plugin_supports_version abipgn_grunixpango_supports_version
#endif

static	UT_uint32 s_iPrevDefaultScreen = 0;
static	UT_uint32 s_iPrevDefaultPrinter = 0;

ABI_PLUGIN_DECLARE("gr_UnixPangoGraphics")
...
int abi_plugin_register (XAP_ModuleInfo * mi)
{
	...
	mi->desc    = "";
	mi->version = "0.1.0.0";
	mi->author  = "Daniel Glassey <danglassey@ntlworld.com";
	mi->usage   = "";
	UT_DEBUGMSG(("abi_plugin_register GR_UnixPangoGraphics\n"));
	...
	if(!pGF->registerClass(GR_UnixPangoGraphics::graphicsAllocator,
						   ...
						   GR_UnixPangoGraphics::s_getClassId()))
	{
		...
		GR_UnixAllocInfo ai(NULL, NULL, pApp);
		//!!!WDG allocinfo?
		GR_Graphics * pG = pApp->newGraphics(GR_UnixPangoGraphics::s_getClassId(), ai);
		...
		if(v1 > v2)
		{
			...
			if(s_iPrevDefaultScreen == GR_UnixPangoGraphics::s_getClassId())
			{
				s_iPrevDefaultScreen = GRID_UNIX;
				pGF->registerAsDefault(GRID_UNIX, true);
			}
			
			if(s_iPrevDefaultPrinter == GR_UnixPangoGraphics::s_getClassId())
			{
				s_iPrevDefaultPrinter = GRID_UNIX;
				pGF->registerAsDefault(GRID_UNIX, false);
			}

			if(!pGF->unregisterClass(GR_UnixPangoGraphics::s_getClassId()
			|| !pGF->registerClass(GR_UnixPangoGraphics::graphicsAllocator,
								   GR_UnixPangoGraphics::graphicsDescriptor,
								   GR_UnixPangoGraphics::s_getClassId())))
			{
				UT_ASSERT_HARMLESS( UT_SHOULD_NOT_HAPPEN );
	...
	pGF->registerAsDefault(GR_UnixPangoGraphics::s_getClassId(), false);
...
int abi_plugin_unregister (XAP_ModuleInfo * mi)
{
	...
	if(pGF->getDefaultClass(true) == GR_UnixPangoGraphics::s_getClassId())
	{
	...
	if(pGF->getDefaultClass(false) == GR_UnixPangoGraphics::s_getClassId())
	{
