...
GR_CocoaFont::GR_CocoaFont(NSFont* font)
    : GR_Font()
    , m_font([font retain])
    , _m_coverage(nullptr)
{
...
GR_CocoaFont::GR_CocoaFont(const GR_CocoaFont& copy)
    : GR_Font(copy)
    , m_font([copy.m_font copy])
    , _m_coverage(nullptr)
{
...
CGFloat GR_CocoaFont::getSize(void) const
{
    return [m_font pointSize];
}

const char* GR_CocoaFont::getName(void) const
{
    return [[m_font fontName] UTF8String];
}

const char* GR_CocoaFont::getFamily() const
{
    return [[m_font familyName] UTF8String];
}

/*
	metrics get cached for efficiency. These calls are MUCH CHEAPER than an AppKit call
	since Font is persistant inside the GR_CocoaFont, the is almost no cost doing that
...
CGFloat GR_CocoaFont::getAscent() const
{
    return [m_font ascender];
}

CGFloat GR_CocoaFont::getDescent() const
{
    return -[m_font descender];
}

CGFloat GR_CocoaFont::getHeight() const
{
    return getAscent() + getDescent();
    ;
}

void GR_CocoaFont::getCoverage(UT_NumberVector& coverage) const
{
    UT_uint32 i, begin;
...
bool GR_CocoaFont::glyphBox(UT_UCS4Char /*g*/, UT_Rect& /*rec*/, GR_Graphics* /*pG*/)
{
	...
	if (!m_LayoutHelper)
	{
		m_LayoutHelper = new GR_CocoaFont_LayoutHelper(m_font);
	}
	if (m_LayoutHelper) {
...
UT_sint32 GR_CocoaFont::measureUnremappedCharForCache(UT_UCSChar) const
{
    // Like on Cairo.
...
GR_CocoaFont::RemapFont GR_CocoaFont::remapFont(NSFont* font)
{
...
UT_UCS4Char GR_CocoaFont::remapChar(UT_UCS4Char charCode, RemapFont rf)
{
...
@implementation GR_CocoaFontFamilyHelper
...
+ (NSMutableDictionary*)fontFamilyHelperDictionary:(NSMutableDictionary*)referenceDictionary
{
    ...
    for (NSUInteger i = 0; i < count; i++) {
        ...
        GR_CocoaFontFamilyHelper* helper = [[GR_CocoaFontFamilyHelper alloc] initWithFontFamilyName:family known:YES];
...
- (id)initWithFontFamilyName:(NSString*)fontFamilyName known:(BOOL)known
{
    ...
    m_FontFamilyName = [fontFamilyName retain];
    ...
    if (known) {
        NSArray* members = [[NSFontManager sharedFontManager] availableMembersOfFontFamily:m_FontFamilyName];
        ...
        for (NSUInteger i = 0; i < m_count; i++) {
            ...
            if ([memberName isEqualToString:@"Regular"] ||
                ...
                m_indexRegular = (NSInteger)i;
            } else if ([memberName isEqualToString:@"Italic"] ||
                [memberName isEqualToString:@"Oblique"] ||
                [memberName isEqualToString:@"Inclined"]) {
                m_indexItalic = (NSInteger)i;
            } else if ([memberName isEqualToString:@"Bold"]) {
                m_indexBold = (NSInteger)i;
            } else if ([memberName isEqualToString:@"Bold Italic"]
                || [memberName isEqualToString:@"BoldItalic"]
                || [memberName isEqualToString:@"Bold Oblique"]
                || [memberName isEqualToString:@"BoldOblique"]
                || [memberName isEqualToString:@"Bold Inclined"]
                || [memberName isEqualToString:@"BoldInclined"]) {
                m_indexBoldItalic = (NSInteger)i;
            }
        }

        NSUInteger indexRegular = (m_indexRegular >= 0) ? ((unsigned)m_indexRegular) : 0;
...
- (void)addFontReferences:(NSMutableDictionary*)referenceDictionary
{
    for (NSUInteger i = 0; i < m_count; i++) {
        ...
        GR_CocoaFontReference* fontRef = [[GR_CocoaFontReference alloc] initWithFontFamily:m_FontFamilyName helper:self index:i];
...
@implementation GR_CocoaFontReference
...
- (id)initWithFontFamily:(NSString*)fontFamily helper:(GR_CocoaFontFamilyHelper*)helper index:(NSUInteger)index
{
    ...
    m_FontFamily = [fontFamily retain];

    m_FontFamilyHelper = [helper retain];
