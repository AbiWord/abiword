...
{
	/*source
		= gdk_pixmap_colormap_create_from_xpm_d(_getDrawable(),NULL,          //Needs reviewing
							&mask, NULL,
							pXPM); */
}
...
/*#if GTK_CHECK_VERSION(2,17,11) || GTK_2_17_DEV_VERSION_WITHOUT_RESET_CLIP    // We don't need it -> Needs Reviewing'
...
void GR_UnixCairoGraphics::saveRectangle(UT_Rect & r, UT_uint32 iIndx)
{
    ...
    cairo_surface_t *s;
    cairo_t *cr;
    s = cairo_image_surface_create (CAIRO_FORMAT_A1, idw, idh);
    cr = cairo_create (s);
    cairo_rectangle(cr,idx,idy,idw,idh);
    cairo_fill(cr);
    cairo_destroy(cr);
	GdkPixbuf * pix = gdk_pixbuf_get_from_surface(
												   s,
...
/*		gdk_draw_pixbuf (_getDrawable(), NULL, p, 0, 0,
    ...
    sub_pixbuf = gdk_pixbuf_new_subpixbuf (p,0,0,-1,-1);
    m_cr = gdk_cairo_create (_getDrawable());
    gdk_cairo_set_source_pixbuf (m_cr,sub_pixbuf,idx,idy);
    cairo_paint(m_cr);
	cairo_restore(m_cr);
