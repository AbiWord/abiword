...
#include "xap_BeOSDlg_History.h"

#include "ut_Rehydrate.h"

/*****************************************************************/

XAP_Dialog * XAP_BeOSDialog_History::static_constructor(XAP_DialogFactory * pFactory,
													 XAP_Dialog_Id id)
{
	XAP_BeOSDialog_History * p = new XAP_BeOSDialog_History(pFactory,id);
	return p;
}

XAP_BeOSDialog_History::XAP_BeOSDialog_History(XAP_DialogFactory * pDlgFactory,
												 XAP_Dialog_Id id)
	: XAP_Dialog_History(pDlgFactory,id)
{
}

XAP_BeOSDialog_History::~XAP_BeOSDialog_History(void)
{
}

void XAP_BeOSDialog_History::runModal(XAP_Frame * pFrame)
{
	UT_ASSERT(pFrame);
...
/*
	...
	2.  Do a case sensitive global replace on the words History and STUB
		in both files. 
