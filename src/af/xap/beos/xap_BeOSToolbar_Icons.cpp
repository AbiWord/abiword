...
/*
	...
	DPRINTF(printf("ICONS: %s -> an array of %d bytes \n", szIconName, sizeofIconData));
	//Translate the stream of data into a BBitmap ... 
	BMemoryIO memio((void *)&pIconData[0], sizeofIconData);
	outbitmap =  BTranslationUtils::GetBitmap(&memio);
*/
	char fname[255];
	//This is really terrible ... oh well 
	strcpy(fname, "../../ap/xp/ToolbarIcons/");
	strcat(fname, szIconName);
	//Replace the last _ with a .
	...
	DPRINTF(printf("Suck bitmap from %s \n", fname));
	outbitmap = BTranslationUtils::GetBitmap(fname);	
	DPRINTF(printf("ICONS: %s in locating translator\n", (outbitmap) ? "SUCCESS" : "FAILURE"));
	return(outbitmap);
}
