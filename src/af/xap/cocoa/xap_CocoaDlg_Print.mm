...
XAP_Dialog * XAP_CocoaDialog_Print::static_constructor(XAP_DialogFactory * pFactory,
													 XAP_Dialog_Id id)
{
	XAP_CocoaDialog_Print * p = new XAP_CocoaDialog_Print(pFactory,id);
	return p;
}
...
{
	m_pCocoaFrame = static_cast<XAP_CocoaFrame *>(pFrame);
...
void XAP_CocoaDialog_Print::_raisePrintDialog(XAP_Frame * pFrame)
{
	...
	gtk_widget_show (separator);
		...
		gtk_widget_show (hbox);
			...
			m_callbackData.frame = (XAP_Frame *) m_pCocoaFrame;
...
void XAP_CocoaDialog_Print::_getGraphics(void)
{
	UT_ASSERT(m_answer == a_OK);
	...
	XAP_CocoaFontManager * fontmgr = unixapp->getFontManager();
	UT_ASSERT(fontmgr);
	...
	if (m_bDoPrintToFile)
	{
		...
		if (!_getPrintToFilePathname(m_pCocoaFrame,bufSuggestedName))
			goto Fail;

		m_pPSGraphics = new PS_Graphics(m_szPrintToFilePathname, m_szDocumentTitle,
										m_pCocoaFrame->getApp()->getApplicationName(),
										fontmgr,
										true, app);
	}
	else
