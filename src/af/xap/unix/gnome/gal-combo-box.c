...
/*
 * gal-combo-box.c - a customizable combobox
...
static int gal_combo_toggle_pressed (GtkToggleButton *tbutton,
				     GalComboBox *combo_box);
static void gal_combo_popup_tear_off (GalComboBox *combo,
				      gboolean set_position);
static void gal_combo_set_tearoff_state (GalComboBox *combo,
					 gboolean torn_off);
static void gal_combo_popup_reparent (GtkWidget *popup, GtkWidget *new_parent,
				      gboolean unrealize);
static gboolean cb_popup_delete (GtkWidget *w, GdkEventAny *event,
			     GalComboBox *combo);
static void gal_combo_tearoff_bg_copy (GalComboBox *combo);
...
deactivate_arrow (GalComboBox *combo_box)
{
	...
	g_signal_handlers_block_matched (arrow,
					 ...
					 0, 0, NULL,
					 gal_combo_toggle_pressed, combo_box);
...
gal_combo_box_popup_hide_unconditional (GalComboBox *combo_box)
{
	...
	if (combo_box->priv->torn_off) {
		GTK_TEAROFF_MENU_ITEM (combo_box->priv->tearable)->torn_off
			= FALSE;
		gal_combo_set_tearoff_state (combo_box, FALSE);
	}
...
gal_combo_box_popup_hide (GalComboBox *combo_box)
{
	if (!combo_box->priv->torn_off)
		...
		/* Both popup and tearoff window present. Get rid of just
                   the popup shell. */
		gal_combo_popup_tear_off (combo_box, FALSE);
		deactivate_arrow (combo_box);
	}
}
...
static int
gal_combo_toggle_pressed (GtkToggleButton *tbutton, GalComboBox *combo_box)
{
	if (tbutton->active)
...
gal_combo_box_init (GalComboBox *combo_box)
{
	...
	g_signal_connect (combo_box->priv->arrow_button, "toggled",
			  G_CALLBACK (gal_combo_toggle_pressed), combo_box);
 ...
 */
static void
gal_combo_popup_tear_off (GalComboBox *combo, gboolean set_position)
{
	int x, y;
	...
	gal_combo_popup_reparent (combo->priv->popup,
 ...
 */
static void
gal_combo_set_tearoff_state (GalComboBox *combo,
			     gboolean  torn_off)
{
	...
	if (combo->priv->torn_off != torn_off) {
		...
		if (combo->priv->torn_off) {
			gal_combo_popup_tear_off (combo, TRUE);
...
/**
 * gal_combo_tearoff_bg_copy
 * @combo_box:  Combo box
 *
 * Copy popup window image to the tearoff window.
 */
static void
gal_combo_tearoff_bg_copy (GalComboBox *combo)
{
	GdkPixmap *pixmap;
	GdkGC *gc;
 ...
 */
static void
gal_combo_popup_reparent (GtkWidget *popup,
			    ...
			    GalComboBox *combo)
{
	...
	if (!combo->priv->torn_off) {
		...
		gal_combo_set_tearoff_state (combo, TRUE);
		if (need_connect)
...
gal_combo_box_set_tearable (GalComboBox *combo, gboolean tearable)
{
	...
	if (tearable){
		...
		gal_combo_set_tearoff_state (combo, FALSE);
