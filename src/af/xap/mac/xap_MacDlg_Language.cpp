...
XAP_Dialog * XAP_MacDialog_Language::static_constructor(XAP_DialogFactory * pFactory,
													 XAP_Dialog_Id id)
{
	XAP_MacDialog_Language * p = new XAP_MacDialog_Language(pFactory,id);
	return p;
}

XAP_MacDialog_Language::XAP_MacDialog_Language(XAP_DialogFactory * pDlgFactory,
												 XAP_Dialog_Id id)
	: XAP_Dialog_Language(pDlgFactory,id)
{
}

XAP_MacDialog_Language::~XAP_MacDialog_Language(void)
{
}

void XAP_MacDialog_Language::runModal(XAP_Frame * pFrame)
{
	UT_ASSERT(pFrame);
...
/*
	...
	2.  Do a case sensitive global replace on the words Language and STUB
		in both files. 
