...
#include "xap_QtEncodingManager.h"
...
XAP_EncodingManager *XAP_EncodingManager::get_instance()
{
	if (_instance == 0)
	{
		...
		_instance = new XAP_QtEncodingManager();
...
static std::string NativeEncodingName;
static std::string NativeSystemEncodingName;
static std::string Native8BitEncodingName;
static std::string NativeNonUnicodeEncodingName;
static std::string NativeUnicodeEncodingName;
static std::string LanguageISOName;
static std::string LanguageISOTerritory;

XAP_QtEncodingManager::XAP_QtEncodingManager() 
{
}
...
const char* XAP_QtEncodingManager::getNativeEncodingName() const
{     
  return NativeEncodingName.c_str(); 
}

const char* XAP_QtEncodingManager::getNativeSystemEncodingName() const
{     
  return NativeSystemEncodingName.c_str(); 
}

const char* XAP_QtEncodingManager::getNative8BitEncodingName() const
{     
  return Native8BitEncodingName.c_str();
}

const char* XAP_QtEncodingManager::getNativeNonUnicodeEncodingName() const
{     
  return NativeNonUnicodeEncodingName.c_str();
}

const char* XAP_QtEncodingManager::getNativeUnicodeEncodingName() const
{     
  return NativeUnicodeEncodingName.c_str(); 
}

const char* XAP_QtEncodingManager::getLanguageISOName() const
{
 	return LanguageISOName.c_str(); 
}

const char* XAP_QtEncodingManager::getLanguageISOTerritory() const
{ 	
  return LanguageISOTerritory.c_str(); 
}

void  XAP_QtEncodingManager::initialize()
{	
	...
	else
	{
		...
		if ((mask & COMPONENT_CODESET) && cs)
		{
			...
			if(!g_ascii_strcasecmp(NativeEncodingName.c_str(), "UTF-8"))
			{
				...
				g_setenv ("LANG", MYLANG.c_str(), TRUE);
				if (mask & COMPONENT_CODESET)
				{
				...
				g_setenv("LANG", OLDLANG.c_str(), TRUE);
			}
