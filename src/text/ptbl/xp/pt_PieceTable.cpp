 ...
 */
bool pt_PieceTable::insertStruxNoUpdateBefore(PL_StruxDocHandle sdh, PTStruxType pts,const PT_AttributePair* attributes )
{
	const pf_Frag_Strux * pfs = static_cast<const pf_Frag_Strux *>(sdh);
...
void pt_PieceTable::deleteHdrFtrStrux(pf_Frag_Strux * pfs)
{
	...
	else
	{
		...
		GQuark qHdrId;
		if(!pAP->getAttribute(PT_ID_ATTRIBUTE_NAME, qHdrId) || !qHdrId)
			return;

		GQuark qHdrType;
		if(!pAP->getAttribute(PT_TYPE_ATTRIBUTE_NAME, qHdrType) || qHdrType)
			return;

		_realDeleteHdrFtrStrux(pfs);
		_fixHdrFtrReferences(qHdrType, qHdrId);
	}
	...
	while(pf)
	{
		...
		if(pf->getType() == pf_Frag::PFT_Object)
		{
			...
			if(pOb->getObjectType() == PTO_Hyperlink)
			{
				...
				GQuark qHref;
				if (!pAP->getAttribute (PT_XLINKHREF_ATTRIBUTE_NAME, qHref) || !qHref)
					return NULL;
