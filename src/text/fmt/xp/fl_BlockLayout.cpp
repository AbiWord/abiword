...
bool	fl_BlockLayout::_doInsertTextSpan(PT_BlockOffset blockOffset, UT_uint32 len)
{
	...
	// now itemize the span
	UT_return_val_if_fail(pG->itemize(text, I), false);
	
	for(UT_uint32 i = 0; i < I.getItemCount() - 1; ++i)
	{
		xxx_UT_DEBUGMSG(("_dopopulateTextSpan: text run: offset %d, len %d\n", curOffset, i));
		UT_uint32 iLen = I.getNthOffset(i+1) - I.getNthOffset(i);
		
		fp_TextRun* pNewRun = new fp_TextRun(this, I.getNthOffset(i)+blockOffset, iLen);
		UT_return_val_if_fail(pNewRun && pNewRun->getType() == FPRUN_TEXT, false);
		
		pNewRun->setDirOverride(m_iDirOverride);
		pNewRun->setScriptType(I.getNthType(i));

		UT_return_val_if_fail(_doInsertRun(pNewRun), false);
	}
