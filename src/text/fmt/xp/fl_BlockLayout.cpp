...
{
	static GrammarChecker * checker = NULL;
	static char szPrevLang[8] = {0};
	...
	if(!szPrevLang[0] || strcmp(pszLang,szPrevLang))
	{
		checker = GrammarManager::instance().requestChecker(pszLang);
...
bool
fl_BlockLayout::_grammarCheckSentence(const UT_UCSChar * sentence,
								UT_uint32 len, UT_uint32 blockPos)
{
	GrammarChecker * checker = _getGrammarChecker (blockPos);

	if (!checker)
		return true;
	
	if (!checker->checkSentence(sentence, len))
		return true;
	
	if (checker->getNMistakes() != 0)
		return false;
	return true;
}

#endif // ENABLE_GRAMMAR
