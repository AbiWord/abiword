...
/*char *b_name = 0;*/
FILE *fontfile = 0;
/* FILE *afmfile, *unifile, *psfontfile = 0; */
...
int create_type42(const char* pszTTFont, FILE * psfontfile)
{
	int fd = 0;
	int i;
	...
	if ((fontfile = fopen(pszTTFont, "r")) == NULL)
	{
		ttf_fail("cannot open font file [%s] for reading", pszTTFont);
		return FALSE;
	}
	...
	if ((fd=open(pszTTFont,O_RDONLY))<0)
	{
		ttf_fail("cannot open font file [%s] for reading", pszTTFont);
		return FALSE;
	}
