...
static IE_Imp_Applix_Sniffer * m_sniffer = 0;
...
int abi_plugin_register (XAP_ModuleInfo * mi)
{
	...
	if (!m_refs && !m_sniffer)
	{
		m_sniffer = new IE_Imp_Applix_Sniffer ();
		m_refs++;
	}
	...
	mi->name = "Applix Importer";
	mi->desc = "Import Applix Documents";
	mi->version = "0.7.15";
...
UT_Error IE_Imp_Applix::_parseFile(FILE * fp)
{
    ...
    while (!feof (fp))
    {
		...
		if (ok)
		{
			len = strlen((char *)buf.getPointer (0));
			// todo: make me more robust
			// grammars? we don't need no stinkin' grammars! ;-(
			tag = s_getTagName((char *)buf.getPointer(0), len);
			if (tag != NOT_A_TAG) 
			{
				_dispatchTag (tag, (char *)buf.getPointer(0), len);
			}
		}
    }
...
IE_Imp_Applix::_applixGetLine (UT_ByteBuf* pBuf, FILE *fp)
{
	...
	do 
	{
		...
		if (lineCount > 0) 
		{
			...
			// ZAP the space
			pBuf->append ((UT_Byte*)&temp[1], contentLen - 1);
		}
		else 
		{
			pBuf->append ((UT_Byte*)temp, contentLen);
		}
		if (eol == '\\')
		{
			// line wrap
	...
	pBuf->append ((UT_Byte*)"", 1);  // add terminal 0
	return true;
}
