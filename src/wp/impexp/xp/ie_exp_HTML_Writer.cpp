...
bool IE_Exp_HTML_Writer::compareStyle(const char * key, const char * value)
{
...
void IE_Exp_HTML_Writer::tagRaw(UT_UTF8String & content)
{
#ifdef HTML_ENABLE_MHTML
    if (m_bQuotedPrintable) content.escapeMIME();
#endif
    // fputs (content.utf8_str (), stdout);
    write(content.utf8_str(), content.byteLength());
    m_iOutputLen += content.byteLength();
}

UT_uint32 IE_Exp_HTML_Writer::tagIndent()
{
    return m_tagStack.getDepth();
}

void IE_Exp_HTML_Writer::tagNewIndent(UT_UTF8String & utf8, UT_uint32 depth)
{
    UT_uint32 i;
...
void IE_Exp_HTML_Writer::tagNewIndent(UT_uint32 extra)
{
    m_utf8_0 = "";
...
void IE_Exp_HTML_Writer::tagOpenClose(const UT_UTF8String & content, bool suppress,
                                            WhiteSpace ws)
{
    if (ws & ws_Pre)
        tagNewIndent();
...
void IE_Exp_HTML_Writer::tagOpen(UT_uint32 tagID, const UT_UTF8String & content,
                                       WhiteSpace ws)
{
    if (ws & ws_Pre)
        tagNewIndent();
...
void IE_Exp_HTML_Writer::tagClose(UT_uint32 tagID, const UT_UTF8String & content,
                                        WhiteSpace ws)
{
    tagClose(tagID);
...
void IE_Exp_HTML_Writer::tagClose(UT_uint32 tagID)
{
    UT_uint32 i = 0;
 ...
 */
void IE_Exp_HTML_Writer::tagOpenBroken(const UT_UTF8String & content,
                                             WhiteSpace ws)
{
    if (ws & ws_Pre)
    {
...
/* use with *extreme* caution! (this is used by images with data-URLs)
 */
void IE_Exp_HTML_Writer::tagCloseBroken(const UT_UTF8String & content, bool suppress,
                                              WhiteSpace ws)
{
    m_utf8_0 = content;
...
UT_uint32 IE_Exp_HTML_Writer::tagTop()
{
    UT_sint32 i = 0;
...
void IE_Exp_HTML_Writer::tagPop()
{
...
void IE_Exp_HTML_Writer::tagPI(const char * target, const UT_UTF8String & content)
{
    tagNewIndent();
...
void IE_Exp_HTML_Writer::tagComment(const UT_UTF8String & content)
{
    tagNewIndent();
...
void IE_Exp_HTML_Writer::tagCommentOpen()
{
    tagNewIndent();
...
void IE_Exp_HTML_Writer::tagCommentClose()
{
    tagNewIndent(2);
...
void IE_Exp_HTML_Writer::styleIndent()
{
    m_utf8_0 = "";
...
void IE_Exp_HTML_Writer::styleOpen(const UT_UTF8String & rule)
{
    styleIndent();
...
void IE_Exp_HTML_Writer::styleClose()
{
    if (m_styleIndent == 0)
    {
...
void IE_Exp_HTML_Writer::styleNameValue(const char * name, const UT_UTF8String & value)
{
    styleIndent();
...
void IE_Exp_HTML_Writer::styleText(const UT_UTF8String & content)
{
    if (m_fdCSS)
...
void IE_Exp_HTML_Writer::textTrusted(const UT_UTF8String & text)
{
    if (text.byteLength())
    {
        m_utf8_0 = text;
...
void IE_Exp_HTML_Writer::textUntrusted(const char * text)
{
    if (text == 0) return;
    if (*text == 0) return;
...
void IE_Exp_HTML_Writer::multiHeader(const UT_UTF8String & title)
{
...
void IE_Exp_HTML_Writer::multiBoundary(bool end)
{
    m_utf8_0 = MYEOL "--";
    m_utf8_0 += s_boundary;
    ...
    write(m_utf8_0.utf8_str(), m_utf8_0.byteLength());
    m_iOutputLen += m_utf8_0.byteLength();
}

void IE_Exp_HTML_Writer::multiField(const char * name, const UT_UTF8String & value)
{
    m_utf8_0 = name;
    m_utf8_0 += ":";
    m_utf8_0 += value;
...
void IE_Exp_HTML_Writer::multiBreak()
{
    m_utf8_0 = MYEOL;
    // fputs (m_utf8_0.utf8_str (), stdout);
    write(m_utf8_0.utf8_str(), m_utf8_0.byteLength());
...
void IE_Exp_HTML_Writer::_outputBegin(PT_AttrPropIndex api)
{
    if (m_bTemplateBody)
    {
        m_bFirstWrite = false;
    ...
    {
        write(sMathSVGScript.utf8_str(), sMathSVGScript.size());
    }
...
void IE_Exp_HTML_Writer::_outputEnd()
{
    if (m_bInBlock) closeTag();
...
void IE_Exp_HTML_Writer::openTag (PT_AttrPropIndex api, PL_StruxDocHandle /*sdh*/)
{
	if (m_bFirstWrite) openSection (api, 0);
...
void IE_Exp_HTML_Writer::closeTag (void)
{
	if (!m_bInBlock) return;

	if (m_bInSpan) closeSpan ();
...
void IE_Exp_HTML_Writer::openSpan (PT_AttrPropIndex api)
{
	if (m_bFirstWrite) openTag (api, 0);
 ...
 class_only:
	...
	else if (m_bInSpan) closeSpan ();
}

void IE_Exp_HTML_Writer::closeSpan ()
{
	if (tagTop () == TT_A)
	{
...
bool IE_Exp_HTML_Writer::_openStyleSheet(UT_UTF8String & css_relative_path)
{
...
void IE_Exp_HTML_Writer::_closeStyleSheet()
{
    if (m_fdCSS)
    {
...
void IE_Exp_HTML_Writer::_populateHeaderStyle()
{
...
void IE_Exp_HTML_Writer::_populateFooterStyle()
{
...
void IE_Exp_HTML_Writer::_outputStyles(const PP_AttrProp * pAP)
{
 ...
 */
void IE_Exp_HTML_Writer::startEmbeddedStrux(void)
{
    if (m_bInSection) closeSection();
...
void IE_Exp_HTML_Writer::openSection(PT_AttrPropIndex api, UT_uint16 iSectionSpecialType)
{
    UT_LocaleTransactor t(LC_NUMERIC, "C");
...
void IE_Exp_HTML_Writer::closeSection(void)
{
    ...
    { // If only the first is true, we have a first-order tag mismatch.  The alternative with not testing the latter is a second-order tag mismatch.
        ...
        //		closeTag (); // We need to investigate the tag stack usage of this, and whether or not we really would rather specify the tag in all cases.
        tagClose(TT_P, "p");
    }
    ...
    // popUnendedStructures(); // Close lists, and possibly other stuff.  Even if it theoretically can span sections, we run a high risk of corrupting the document.
 ...
 */
bool IE_Exp_HTML_Writer::_inherits(const char * style, const char * from)
{
...
UT_uint32 IE_Exp_HTML_Writer::listDepth()
{
    return static_cast<UT_uint32> (m_utsListType.getDepth());
}

UT_uint32 IE_Exp_HTML_Writer::listType()
{
    UT_sint32 i = 0;
...
void IE_Exp_HTML_Writer::listPush(UT_uint32 type, const char * /*ClassName*/)
{
    if (tagTop() == TT_LI)
    {
        m_utf8_1 = MYEOL;
...
void IE_Exp_HTML_Writer::listPop()
{
    if (tagTop() == TT_SPAN)
    {
...
void IE_Exp_HTML_Writer::listPopToDepth(UT_uint32 depth)
{
 ...
 */
void IE_Exp_HTML_Writer::popUnendedStructures(void)
{
    if (m_iListDepth)
        listPopToDepth(0);
}
...
void IE_Exp_HTML_Writer::_fillColWidthsVector(void)
{
    // make sure any unit conversions are correct
    UT_LocaleTransactor t(LC_NUMERIC, "C");
...
void IE_Exp_HTML_Writer::openTable(PT_AttrPropIndex api)
{
    // make sure any unit conversions are correct
    UT_LocaleTransactor t(LC_NUMERIC, "C");
    ...
    if (m_bInBlock) closeTag(); // HTML does not make it any more desirable to embed a table in a <p> than AbiWord.
...
void IE_Exp_HTML_Writer::_setCellWidthInches(void)
{
...
void IE_Exp_HTML_Writer::openCell(PT_AttrPropIndex api)
{
    UT_LocaleTransactor t(LC_NUMERIC, "C");
    ...
    if (m_TableHelper.getNestDepth() < 1) openTable(api);
...
void IE_Exp_HTML_Writer::closeCell()
{
    ...
    /* The number of times popUnendedStructures has to be used may be indicative of serious fundamental flows in the
    ...
    popUnendedStructures();

    m_utf8_1 = "td";
    tagClose(TT_TD, m_utf8_1);
}

void IE_Exp_HTML_Writer::openRow (PT_AttrPropIndex api)
{
	UT_LocaleTransactor t(LC_NUMERIC, "C");
...
void IE_Exp_HTML_Writer::closeTable ()
{
	m_utf8_1 = "tbody";
...
void IE_Exp_HTML_Writer::openPosImage(PT_AttrPropIndex api)
{
    const PP_AttrProp * pAP = NULL;
    bool bHaveProp = m_pDocument->getAttrProp(api, &pAP);
    if (!bHaveProp || (pAP == 0)) return;
    ...
    if (pAP->getAttribute(PT_STRUX_IMAGE_DATAID, (const gchar *&) pszDataID) && pszDataID)
        handleImage(pAP, pszDataID, true, false);

}

void IE_Exp_HTML_Writer::openTextBox(PT_AttrPropIndex api)
{
    const PP_AttrProp * pAP = NULL;
    bool bHaveProp = m_pDocument->getAttrProp(api, &pAP);
    if (!bHaveProp || (pAP == 0)) return;
    ...
    if (m_bInTextBox)
        closeTextBox(); // Fortunately for the html exporter, abi does not permit nested frames.
...
void IE_Exp_HTML_Writer::closeTextBox()
{
...
void IE_Exp_HTML_Writer::outputData(const UT_UCSChar * data, UT_uint32 length)
{
    if (!m_bInBlock) return;
...
IE_Exp_HTML_Writer::IE_Exp_HTML_Writer(PD_Document * pDocument, IE_Exp_HTML * pie, bool bClipBoard,
...
IE_Exp_HTML_Writer::~IE_Exp_HTML_Writer()
{
...
void IE_Exp_HTML_Writer::writeImageBase64(const UT_ByteBuf * pByteBuf)
{
    char buffer[75];
    char * bufptr = 0;
    size_t buflen;
...
bool IE_Exp_HTML_Writer::getPropertySize(const PP_AttrProp * pAP, const gchar* szWidthProp, const gchar* szHeightProp,
...
UT_UTF8String IE_Exp_HTML_Writer::getStyleSizeString(const gchar * szWidth, double widthPercentage, UT_Dimension widthDim,
...
void IE_Exp_HTML_Writer::handleEmbedded(PT_AttrPropIndex api)
{
    const PP_AttrProp * pAP = 0;
    bool bHaveProp = m_pDocument->getAttrProp(api, &pAP);
    ...
    handleEmbedded(pAP, szDataID, pByteBuf, mimeType);
}

void IE_Exp_HTML_Writer::handleEmbedded(const PP_AttrProp * pAP, const gchar * szDataID, const UT_ByteBuf* pByteBuf, const std::string mimeType)
{
    ...
    if (!getPropertySize(pAP, "width", "height", &szWidth, widthPercentage, &szHeight))
        return;
    UT_DEBUGMSG(("Size of object: %sx%s\n", szWidth ? szWidth : "(null)", szHeight ? szHeight : "(null)"));
    m_utf8_1 += " " + getStyleSizeString(szWidth, widthPercentage, DIM_MM, szHeight, DIM_MM);
    ...
    else
    {
        ...
        imgDataID += szDataID;
        handleImage(pAP, imgDataID.utf8_str(), false, false);
    }
...
void IE_Exp_HTML_Writer::handleImage(PT_AttrPropIndex api)
{
    const PP_AttrProp * pAP = 0;
    bool bHaveProp = m_pDocument->getAttrProp(api, &pAP);
    ...
    handleImage(pAP, szDataID, false, false);
}

void IE_Exp_HTML_Writer::handleImage(const PP_AttrProp * pAP, const char * szDataID, bool bIsPositioned, bool bIgnoreSize)
{
    UT_LocaleTransactor t(LC_NUMERIC, "C");
    ...
    if (mimeType == "image/svg+xml")
    {
        // We export SVGs as embedded objects. See handleEmbedded() for details on why this is.
        handleEmbedded(pAP, szDataID, pByteBuf, mimeType);
        return;
    }
    ...
    if (!bIgnoreSize)
    {
        if (!getPropertySize(pAP, !bIsPositioned ? "width" : "frame-width", "height", &szWidth, widthPercentage, &szHeight))
            return;
...
void IE_Exp_HTML_Writer::handlePendingImages()
{
...
void IE_Exp_HTML_Writer::handleField(const PX_ChangeRecord_Object * pcro,
...
void IE_Exp_HTML_Writer::handleHyperlink(PT_AttrPropIndex api)
{
    m_utf8_1 = "a";
...
void IE_Exp_HTML_Writer::handleAnnotationMark(PT_AttrPropIndex api)
{
    m_utf8_1 = "a";
...
void IE_Exp_HTML_Writer::handleBookmark(PT_AttrPropIndex api)
{
    m_utf8_1 = "a";
...
void IE_Exp_HTML_Writer::handleMath(PT_AttrPropIndex api)
{
    const PP_AttrProp * pAP = 0;
    bool bHaveProp = (api ? (m_pDocument->getAttrProp(api, &pAP)) : false);
    ...
    if (get_MathML_Render_PNG())
    {
        UT_UTF8String imgDataID = "snapshot-png-";
        imgDataID += szDataID;
        handleImage(pAP, imgDataID.utf8_str(), false, true);
    }
    else
    {
        m_utf8_1 = "a";
...
void IE_Exp_HTML_Writer::handleMetaTag(const char * key, UT_UTF8String & value)
{
    m_utf8_1 = "meta name=\"";
    m_utf8_1 += key;
    m_utf8_1 += "\" content=\"";
...
void IE_Exp_HTML_Writer::handleMeta()
{
    if (!m_pie->isCopying())
    {
        ...
        for (val = cursor.first(); cursor.is_valid(); val = cursor.next())
        {
            if (val)
            {
                ...
                if (stringval->size() > 0)
                {
                    handleMetaTag(cursor.key().c_str(), stringval->c_str()));
                }
            }
#endif
...
void IE_Exp_HTML_Writer::emitTOC(PT_AttrPropIndex api)
{
    if (m_toc)
    {
        ...
        if (bHaveProp && pAP && pAP->getProperty("toc-heading", szValue)) // user-defined TOC heading
        {
            ...
            //outputData() below makes escapeXML() redundant here
        }
        else
        {
        ...
        if (bEmitHeading)
        {
            ...
            m_bInBlock = true;
            outputData(tocHeading.ucs4_str(), tocHeading.length());
            m_bInBlock = false;
            tagClose(TT_H2, "h2");
        }
        ...
        for (int i = 0; i < m_toc->getNumTOCEntries(); i++)
        {
            ...
            m_bInBlock = true;
            write(tocLink.utf8_str(), tocLink.byteLength());
            outputData(tocLevelText.ucs4_str(), tocLevelText.length());
            outputData(tocText.ucs4_str(), tocText.length());
            write("</a>", 4);
            m_bInBlock = false;
            tagClose(TT_P, "p");
        }
...
void IE_Exp_HTML_Writer::doEndnotes()
{
    //
    // Output Endnotes
    //
...
void IE_Exp_HTML_Writer::doFootnotes()
{
    //
    // Output footnotes
    //
...
void IE_Exp_HTML_Writer::doAnnotations()
{
    //
    // Output Annotations
    //
    ...
    for (i = 0; i < nAnnotations; i = i + 1)
    {
        ...
        write(sAnnTag.utf8_str(), sAnnTag.byteLength());
//        m_pDocument->tellListenerSubset(this, pDocRange);
        m_bInAFENote = false;
        m_bInAnnotation = false;
        closeTag(); // close the annotation <p>
    }
    UT_VECTOR_PURGEALL(PD_DocumentRange *, m_vecAnnotations);
}


void IE_Exp_HTML_Writer::addFootnote(PD_DocumentRange * pDocRange)
{
	m_vecFootnotes.addItem(pDocRange);
}

void IE_Exp_HTML_Writer::addEndnote(PD_DocumentRange * pDocRange)
{
	m_vecEndnotes.addItem(pDocRange);
}


void IE_Exp_HTML_Writer::addAnnotation(PD_DocumentRange * pDocRange)
{
	m_vecAnnotations.addItem(pDocRange);
}

UT_uint32 IE_Exp_HTML_Writer::getNumAnnotations(void)
{
	return m_vecAnnotations.getItemCount();
}

UT_uint32 IE_Exp_HTML_Writer::getNumFootnotes(void)
{
	return m_vecFootnotes.getItemCount();
}

UT_uint32 IE_Exp_HTML_Writer::getNumEndnotes(void)
{
	return m_vecEndnotes.getItemCount();
}

void IE_Exp_HTML_Writer::setHaveHeader()
{
	m_bHaveHeader = true;
}

void IE_Exp_HTML_Writer::setHaveFooter()
{
	m_bHaveFooter = true;
}

void IE_Exp_HTML_Writer::write(const char *data, UT_uint32 size)
{
	if (m_bIndexFile)
	{
