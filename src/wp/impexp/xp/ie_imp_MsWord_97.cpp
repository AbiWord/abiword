				  ...
				  void *prop, int dirty)
{
	...
	if(apap->ilfo)
	{
		...
		*/
		...
		// need to make sure future autogenerated list id's will be greater than
		// this
		bool bOK = getDoc()->setMinUID(UT_UniqueId::List, myListId+1);
		if(!bOK)
		{
			UT_DEBUGMSG(("List id %d already in use\n",myListId));
			UT_ASSERT( UT_SHOULD_NOT_HAPPEN );
		}
