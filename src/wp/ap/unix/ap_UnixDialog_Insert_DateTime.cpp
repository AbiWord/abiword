...
void AP_UnixDialog_Insert_DateTime::event_OK(void)
{
	...
	gint indexdata = GPOINTER_TO_INT(gtk_object_get_data(G_OBJECT(listitem->data), LIST_ITEM_INDEX_KEY));
...
void AP_UnixDialog_Insert_DateTime::_populateWindowData(void)
{
    ...
    for (i = 0; InsertDateTimeFmts[i] != NULL; i++)
	{
		...
		gtk_object_set_data(G_OBJECT(listitem), LIST_ITEM_INDEX_KEY, GINT_TO_POINTER(i));
