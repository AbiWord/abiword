...
void AP_UnixDialog_Columns::runModal(XAP_Frame * pFrame)
{
    ...
    gtk_signal_handler_block(G_OBJECT(m_wSpaceAfterEntry), m_iSpaceAfterID);
	gtk_entry_set_text( GTK_ENTRY(m_wSpaceAfterEntry),getSpaceAfterString() );
	gtk_signal_handler_unblock(G_OBJECT(m_wSpaceAfterEntry),m_iSpaceAfterID);

    gtk_signal_handler_block(G_OBJECT(m_wMaxColumnHeightEntry), m_iMaxColumnHeightID);
	gtk_entry_set_text( GTK_ENTRY(m_wMaxColumnHeightEntry),getHeightString() );
	gtk_signal_handler_unblock(G_OBJECT(m_wMaxColumnHeightEntry), m_iMaxColumnHeightID);
...
void AP_UnixDialog_Columns::readSpin(void)
{
	...
	gtk_signal_handler_block(G_OBJECT(m_wtoggleOne), 
							 m_oneHandlerID);
	gtk_signal_handler_block(G_OBJECT(m_wtoggleTwo), 
							 m_twoHandlerID);
	gtk_signal_handler_block(G_OBJECT(m_wtoggleThree), 
							 m_threeHandlerID);
	...
	gtk_signal_handler_unblock(G_OBJECT(m_wtoggleOne), 
							   m_oneHandlerID);
	gtk_signal_handler_unblock(G_OBJECT(m_wtoggleTwo), 
							   m_twoHandlerID);
	gtk_signal_handler_unblock(G_OBJECT(m_wtoggleThree), 
							   m_threeHandlerID);
	setColumns( val );
...
void AP_UnixDialog_Columns::event_Toggle( UT_uint32 icolumns)
{
	...
	gtk_signal_handler_block(G_OBJECT(m_wtoggleThree), 
		...
		// DOM: TODO: rewrite me
	gtk_signal_handler_block(G_OBJECT(m_wSpin), 
							 m_spinHandlerID);
	gtk_spin_button_set_value( GTK_SPIN_BUTTON(m_wSpin), (gfloat) icolumns);
	gtk_signal_handler_unblock(G_OBJECT(m_wSpin), 
							   m_spinHandlerID);
...
void AP_UnixDialog_Columns::_constructWindowContents(GtkWidget * windowColumns)
{
	...
	gtk_object_set_data_full (G_OBJECT (windowColumns), "wPreviewArea", wPreviewArea,
...
void AP_UnixDialog_Columns::_connectsignals(void)
{
	...
	// the control buttons
	m_oneHandlerID = gtk_signal_connect(G_OBJECT(m_wtoggleOne),
					   "clicked",
					   GTK_SIGNAL_FUNC(s_one_clicked),
					   (gpointer) this);

	m_twoHandlerID = gtk_signal_connect(G_OBJECT(m_wtoggleTwo),
					   "clicked",
					   GTK_SIGNAL_FUNC(s_two_clicked),
					   (gpointer) this);

	m_threeHandlerID = gtk_signal_connect(G_OBJECT(m_wtoggleThree),
					   "clicked",
					   GTK_SIGNAL_FUNC(s_three_clicked),
					   (gpointer) this);


	m_spinHandlerID = gtk_signal_connect(G_OBJECT(m_wSpin),
					   "changed",
					   GTK_SIGNAL_FUNC(s_spin_changed),
					   (gpointer) this);


	gtk_signal_connect(G_OBJECT(m_wSpaceAfterSpin),
					   "changed",
					  GTK_SIGNAL_FUNC(s_SpaceAfterSpin_changed),
					   (gpointer) this);


	gtk_signal_connect(G_OBJECT(m_wMaxColumnHeightSpin),
					   "changed",
					  GTK_SIGNAL_FUNC(s_HeightSpin_changed),
					   (gpointer) this);

	m_iSpaceAfterID = gtk_signal_connect(G_OBJECT(m_wSpaceAfterEntry),
					   "changed",
					  GTK_SIGNAL_FUNC(s_SpaceAfterEntry_changed),
					   (gpointer) this);


	m_iMaxColumnHeightID = gtk_signal_connect(G_OBJECT(m_wMaxColumnHeightEntry),
					   "changed",
					  GTK_SIGNAL_FUNC(s_MaxHeightEntry_changed),
					   (gpointer) this);

	gtk_signal_connect(G_OBJECT(m_wlineBetween),
					   "clicked",
					   GTK_SIGNAL_FUNC(s_line_clicked),
					   (gpointer) this);

	gtk_signal_connect(G_OBJECT(m_wbuttonOk),
					   "clicked",
					   GTK_SIGNAL_FUNC(s_ok_clicked),
					   (gpointer) this);
	
	gtk_signal_connect(G_OBJECT(m_wbuttonCancel),
					   "clicked",
					   GTK_SIGNAL_FUNC(s_cancel_clicked),
					   (gpointer) this);

	// the expose event of the preview
	             gtk_signal_connect(G_OBJECT(m_wpreviewArea),
					   "expose_event",
