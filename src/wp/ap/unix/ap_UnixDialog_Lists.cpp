...
void  AP_UnixDialog_Lists::styleChanged(gint type)
{
	...
	if(type == 0)
	{
		...
		gtk_signal_handler_block(  G_OBJECT(m_wListStyleBox),m_iStyleBoxID  );
		gtk_signal_handler_block_by_data(  G_OBJECT(m_wListStyleBox), (gpointer) this );
		gtk_option_menu_set_menu (GTK_OPTION_MENU (m_wListStyleBox), 
								  m_wListStyleNone_menu);
		gtk_signal_handler_unblock(  G_OBJECT(m_wListStyleBox),m_iStyleBoxID  );
...
void AP_UnixDialog_Lists::_fillFontMenu(GtkWidget* menu)
{
	...
	gtk_signal_connect (G_OBJECT (glade_menuitem), "activate",
...
GtkWidget *AP_UnixDialog_Lists::_constructWindowContents (void)
{
	...
	gtk_object_set_user_data(G_OBJECT(menu_none),GINT_TO_POINTER(0));
	...
	gtk_object_set_user_data(G_OBJECT(menu_bull),GINT_TO_POINTER(1));
	...
	gtk_object_set_user_data(G_OBJECT(menu_num),GINT_TO_POINTER(2));
...
void AP_UnixDialog_Lists::_fillNoneStyleMenu( GtkWidget *listmenu)
{
	...
	gtk_widget_show (glade_menuitem);
	gtk_object_set_user_data(G_OBJECT(glade_menuitem),GINT_TO_POINTER(
		(gint) NOT_A_LIST ));
...
void AP_UnixDialog_Lists::_connectSignals(void)
{
	...
	gtk_signal_connect (G_OBJECT (m_wApply), "clicked",
						GTK_SIGNAL_FUNC (s_applyClicked), this);
	gtk_signal_connect (G_OBJECT (m_wClose), "clicked",
						GTK_SIGNAL_FUNC (s_closeClicked), this);
	gtk_signal_connect (G_OBJECT (m_wCustomLabel), "clicked",
						GTK_SIGNAL_FUNC (s_customChanged), this);
	gtk_signal_connect (G_OBJECT (m_wMenu_None), "activate",
						GTK_SIGNAL_FUNC (s_styleChangedNone), this);
	gtk_signal_connect (G_OBJECT (m_wMenu_Bull), "activate",
						GTK_SIGNAL_FUNC (s_styleChangedBullet), this);
	gtk_signal_connect (G_OBJECT (m_wMenu_Num), "activate",
						GTK_SIGNAL_FUNC (s_styleChangedNumbered), this);
        gtk_signal_connect (G_OBJECT (m_oStartSpin_adj), "value_changed",
						GTK_SIGNAL_FUNC (s_valueChanged), this);
	m_iDecimalEntryID = gtk_signal_connect (G_OBJECT (m_wDecimalEntry), "changed",
										 GTK_SIGNAL_FUNC (s_valueChanged), this);
	m_iAlignListSpinID = gtk_signal_connect (G_OBJECT (m_oAlignList_adj), "value_changed",
						GTK_SIGNAL_FUNC (s_valueChanged), this);
	m_iIndentAlignSpinID = gtk_signal_connect (G_OBJECT (m_oIndentAlign_adj), "value_changed",
						GTK_SIGNAL_FUNC (s_valueChanged), this);
	m_iDelimEntryID = gtk_signal_connect (G_OBJECT (GTK_ENTRY(m_wDelimEntry)), "changed",
										  GTK_SIGNAL_FUNC (s_valueChanged), this);

	m_iStyleBoxID = gtk_signal_connect (G_OBJECT(m_wListStyleBox),
					    "configure_event",
					    GTK_SIGNAL_FUNC (s_typeChanged), 
					    this);
...
void AP_UnixDialog_Lists::loadXPDataIntoLocal(void)
{
  ...
  //
	gtk_signal_handler_block(  G_OBJECT(m_oAlignList_adj), m_iAlignListSpinID);
	gtk_signal_handler_block(  G_OBJECT(m_oIndentAlign_adj), m_iIndentAlignSpinID);

	gtk_signal_handler_block(  G_OBJECT(m_wDecimalEntry), m_iDecimalEntryID);
	gtk_signal_handler_block(  G_OBJECT(m_wDelimEntry), m_iDelimEntryID );
	gint i;
	//
	...
	gtk_signal_handler_unblock(  G_OBJECT(m_oAlignList_adj), m_iAlignListSpinID);
	gtk_signal_handler_unblock(  G_OBJECT(m_oIndentAlign_adj), m_iIndentAlignSpinID);
	gtk_signal_handler_unblock(  G_OBJECT(m_wDelimEntry), m_iDelimEntryID );
	gtk_signal_handler_unblock(  G_OBJECT(m_wDecimalEntry), m_iDecimalEntryID);
	//
