...
void AP_Dialog_InsertHyperlink::setDoc(FV_View * pView)
{
	...
	if(!m_pHyperlink)
	{
		...
		UT_UCSChar * pSelection = pView->getSelectionText();
		UT_ASSERT(pSelection);

		m_pHyperlink = new XML_Char [UT_UCS_strlen(pSelection)+1];
		UT_UCS_strcpy_to_char(m_pHyperlink, pSelection);
