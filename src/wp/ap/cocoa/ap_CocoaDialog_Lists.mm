...
XAP_Dialog * AP_CocoaDialog_Lists::static_constructor(XAP_DialogFactory * pFactory, XAP_Dialog_Id id)
{
	AP_CocoaDialog_Lists * p = new AP_CocoaDialog_Lists(pFactory,id);
	return p;
}
...
static void s_customChanged (GtkWidget * widget, AP_CocoaDialog_Lists * me)
{
  	me->setDirty();
	me->customChanged ();
}
...
static void s_styleChangedNone (GtkWidget * widget, AP_CocoaDialog_Lists * me)
{
  	me->setDirty();
	me->styleChanged ( 0 );
}
 ...
 */
static void s_typeChanged (GtkWidget * widget, AP_CocoaDialog_Lists * me)
{
	if(me->dontUpdate())
		return;
  	me->setDirty();
 ...
 */
static void s_valueChanged (GtkWidget * widget, AP_CocoaDialog_Lists * me)
{
	if(me->dontUpdate())
		return;
  	me->setDirty();
...
static void s_applyClicked (GtkWidget * widget, AP_CocoaDialog_Lists * me)
{
	me->applyClicked();
}

static void s_closeClicked (GtkWidget * widget, AP_CocoaDialog_Lists * me)
{
	me->destroy();
}

static void s_deleteClicked (GtkWidget * widget, gpointer /* data */, AP_CocoaDialog_Lists * me)
{
	me->destroy();
}
...
//
	...
	// make a new Cocoa GC
	m_pPreviewWidget = new GR_CocoaGraphics(m_wPreviewArea->window, unixapp->getFontManager(), m_pApp);
...
void  AP_CocoaDialog_Lists::setListTypeFromWidget(void)
{
	...
	setNewListType((List_Type) GPOINTER_TO_INT(g_object_get_user_data(G_OBJECT(wlisttype))));
}
...
GtkWidget * AP_CocoaDialog_Lists::_constructWindow (void)
{
	GtkWidget *contents;
...
void AP_CocoaDialog_Lists::_fillFontMenu(GtkWidget* menu)
{
	...
	g_object_set_user_data (G_OBJECT (glade_menuitem), GINT_TO_POINTER (0));
	...
	for(i = 0; i < nfonts; i++)
	{
		...
		gtk_widget_show (glade_menuitem);
		g_object_set_user_data (G_OBJECT (glade_menuitem), GINT_TO_POINTER (i + 1));
		gtk_menu_append (GTK_MENU (menu), glade_menuitem);
		g_signal_connect (G_OBJECT (glade_menuitem), "activate",
							G_CALLBACK (s_valueChanged), this);
	}
...
GtkWidget *AP_CocoaDialog_Lists::_constructWindowContents (void)
{
	GtkWidget *vbox2;
	GtkWidget *hbox2;
	GtkWidget *vbox4;
	...
	g_object_set_user_data(G_OBJECT(menu_none),GINT_TO_POINTER(0));
	...
	g_object_set_user_data(G_OBJECT(menu_bull),GINT_TO_POINTER(1));
	...
	g_object_set_user_data(G_OBJECT(menu_num),GINT_TO_POINTER(2));
...
/*
  ...
  Code stolen from xap_CocoaDialog_Insert_Symbol */
...
void AP_CocoaDialog_Lists::_fillNoneStyleMenu( GtkWidget *listmenu)
{
	GtkWidget *glade_menuitem;
	const XAP_StringSet * pSS = m_pApp->getStringSet();
	...
	gtk_widget_show (glade_menuitem);
	g_object_set_user_data(G_OBJECT(glade_menuitem),GINT_TO_POINTER(
		(gint) NOT_A_LIST ));
...
void AP_CocoaDialog_Lists::_fillNumberedStyleMenu( GtkWidget *listmenu)
{
	GtkWidget *glade_menuitem;
	const XAP_StringSet * pSS = m_pApp->getStringSet();
...
void AP_CocoaDialog_Lists::_fillBulletedStyleMenu( GtkWidget *listmenu)
{
	GtkWidget *glade_menuitem;
	const XAP_StringSet * pSS = m_pApp->getStringSet();
...
void AP_CocoaDialog_Lists::_connectSignals(void)
{
...
//
	...
	gint ifont =  GPOINTER_TO_INT(g_object_get_user_data(G_OBJECT(wfont)));
	if(ifont == 0)
	{
		copyCharToFont("NULL");
	}
