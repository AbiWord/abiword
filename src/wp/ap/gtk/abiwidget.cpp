...
abi_widget_render_page_to_image(AbiWidget *abi, int iPage)
{
	...
	cairo_surface_t*  pPixmap = gdk_window_create_similar_surface(pVG->getWindow(),CAIRO_CONTENT_COLOR_ALPHA,iWidth,iHeight); //removed GdkPixmap and gdk_pixmap_new
	      ...
	      GtkWidget    *widget)
{
	...
	ABI_WIDGET(container)->child = gtk_bin_get_child(GTK_BIN (container));   //Removed GTK_BIN (container)->child
}
...
{
	if (!gtk_bin_get_child(GTK_BIN (container)))  //Removed GTK_BIN (container)->child
...
abi_widget_init (AbiWidget * abi)
{
	...
	//GTK_WIDGET_SET_FLAGS (abi, GTK_CAN_FOCUS | GTK_RECEIVES_DEFAULT |GTK_CAN_DEFAULT );   //not needed ->Review
						  ...
						  GtkAllocation *allocation)
{
    ...
    GtkAllocation n_allocation;
	gtk_widget_get_allocation (widget, &n_allocation);     //Removed widget->allocation = *allocation;
    x = n_allocation.x;
    y = n_allocation.y;
    state = gtk_widget_get_state_flags (GTK_WIDGET (widget));
    context = gtk_widget_get_style_context(GTK_WIDGET (widget));
    gtk_style_context_get_padding (context, state, &padding);
	gint border_width = gtk_container_get_border_width(GTK_CONTAINER (widget));  //Replaced GTK_CONTAINER (widget)->border_width
	//gint xthickness = gtk_widget_style_get_property(GTK_WIDGET (widget))->padding.left; //Review
	//gint ythickness = gtk_widget_get_style_context(GTK_WIDGET (widget))->padding.top; //Review
    ...
    if (gtk_widget_get_realized(widget))
    {
		...
		gdk_window_move_resize(gtk_widget_get_window(widget),
							   tmp_allocation.x + border_width,
							   tmp_allocation.y + border_width,
                               tmp_allocation.width - border_width*2,
							   tmp_allocation.height - border_width*2);
/*
        if (abi ->child)
		{
			child_allocation.x = xthickness;
			...
			child_allocation.width = MAX (1,
										  (gint)gtk_widget_get_allocated_width(widget) - child_allocation.x * 2 - border_width * 2);
			child_allocation.height = MAX (1, 
										  (gint)gtk_widget_get_allocated_height(widget) - child_allocation.y * 2 - border_width * 2);
			gtk_widget_size_allocate (ABI_WIDGET ((widget)->child), &child_allocation); // should be a child -> review
			} */
}
}
...
abi_widget_realize (GtkWidget * widget)
{
	...
	TEST: gtk_widget_set_can_focus(widget, TRUE);
	abi = ABI_WIDGET(widget);
    GtkAllocation allocation;
	...
	//attributes.colormap = gtk_widget_get_visual (widget); // not needed -> review
	
	attributes_mask = GDK_WA_X | GDK_WA_Y | GDK_WA_VISUAL /*| GDK_WA_COLORMAP*/;

	//gtk_widget_get_window(widget) = gdk_window_new (gtk_widget_get_parent_window (widget), &attributes, attributes_mask);
	gdk_window_set_user_data (gtk_widget_get_window(widget), abi);

	gtk_widget_style_attach(widget);
	//gtk_style_set_background (widget->style, widget->window, GTK_STATE_NORMAL); //used the below one
...
abi_widget_class_init (AbiWidgetClass *abi_class)
{
	...
	object_class = (GtkWidgetClass *)abi_class;
	...
	//widget_class->size_request  = abi_widget_size_request;   to be implemented via requizition
