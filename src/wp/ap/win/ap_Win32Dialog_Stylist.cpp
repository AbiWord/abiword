...
BOOL AP_Win32Dialog_Stylist::_onInitDialog(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	...
	HWND hTree = GetDlgItem(m_hDlg, AP_RID_DIALOG_STYLIST_TREE_STYLIST);
	hTreeProc = (WHICHPROC) GetWindowLongW(hTree, GWL_WNDPROC); // save off our prior callback
	SetWindowLongW(hTree, GWL_WNDPROC, (LONG)s_treeProc); // tie the treeview to the new callback
...
{		
	if (msg == WM_LBUTTONDBLCLK)
	{
		...
		AP_Win32Dialog_Stylist * pThis = (AP_Win32Dialog_Stylist *)GetWindowLongW(hWnd,GWL_USERDATA);
