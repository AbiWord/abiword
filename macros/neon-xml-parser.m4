...
AC_ARG_ENABLE([libxml2],
 	[  --enable-libxml2         force use of libxml2 ],
 	[abi_force_libxml2=$enableval],
 	[abi_force_libxml2=no])

AC_ARG_ENABLE([expat],
 	[  --enable-expat         force use of expat ],
 	[abi_force_expat=$enableval],
 	[abi_force_expat=no])
	...
	if test "$withval" != "no"; then
		...
		*/libexpat.la)
		...
		if test -r "$withval/expat.h"; then
			...
			if test -z "$abi_using_libtool_expat"; then
				XML_LIBS="-L$withval -lexpat"
			fi

			abi_xml_parser_message="expat in $withval"
			abi_found_parser="yes"
		fi
...
if test "$abi_found_parser" = "no" -a "$abi_force_libxml2" = "no"; then
