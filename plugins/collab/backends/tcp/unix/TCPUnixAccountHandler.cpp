...
void TCPUnixAccountHandler::embedDialogWidgets(void* pEmbeddingParent)
{
	...
	server_button = gtk_radio_button_new_with_label(NULL, _("Accept incoming connections"));
	gtk_box_pack_start(GTK_BOX(vbox), server_button, TRUE, TRUE, 0);
	
	// join a session
	client_button = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(server_button), _("Connect to a server"));
	...
	// host	
	GtkWidget* server_label = gtk_label_new(_("Address:"));
	...
	// ssl
	ssl_button = gtk_check_button_new_with_label(_("Use a secure connection (SSL)"));
	...
	// autoconnect
	autoconnect_button = gtk_check_button_new_with_label(_("Connect on application startup"));
